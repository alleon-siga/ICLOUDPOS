...
class unidades_has_precio_model extends CI_Model {
    ...
    //Obtener precio por id y unidad (18-10-2018) Carlos Camargo
    function get_un_h_precio($id_producto, $nombre_unidad) {
        $this->db->select('uhp.id_producto,uhp.precio,u.nombre_unidad');
        $this->db->from(' unidades_has_precio AS  uhp');
        $this->db->join(' unidades as u', 'u.id_unidad= uhp.id_unidad');
        $this->db->where('id_precio="3"');
        $this->db->where('uhp.id_producto',$id_producto);        
        $this->db->where('u.nombre_unidad',$nombre_unidad);
...
GROUP BY id_producto, precios.id_precio,unidades_has_precio.`id_unidad` ORDER BY unidades_has_producto.orden asc, grupos.nombre_grupo asc");
    ...
    public function traer_by($select = false, $from = false, $join = false, $campos_join = false, $tipo_join, $where = false, $group = false, $order = false, $retorno = false) {
