...
class linea extends MY_Controller {
    ...
    function eliminar()
    {
        ...
        $data['resultado'] = $this->lineas_model->verifProdLin($linea);

        if($data['resultado'] != 'relacion_linea'){
            $data['resultado'] = $this->lineas_model->update_lineas($linea);
            ...
            if ($data['resultado'] != FALSE) {
               ...
               // $this->session->set_flashdata('error', 'Ha ocurrido un error al eliminar la linea');
                ...
                $json['warning']= 'No se puede eliminar la linea, tiene productos relacionados';
        }
        echo json_encode($json);
    }
