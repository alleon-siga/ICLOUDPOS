            ...
            <table class='table table-striped table-media dataTable table-bordered'>
                ...
                <?php if (count($lstUsuario) > 0): ?>
                    <?php foreach ($lstUsuario as $usu): ?>
                        <tr>
                            <td style="text-align: center;"><?php echo $usu->id; ?></td>
                            ...
                            <?php //if ($usu->esSuper != 1){ ?>
                                <div class="btn-group">
                                    <a style="cursor:pointer;"
                                       onclick="editar(<?php echo $usu->id; ?>,'<?php echo $usu->username; ?>')"
                                       class='btn btn-default tip' title="Editar">
                                        <i class="fa fa-edit"></i>
                                    </a>
                                    <a href="#" class='btn btn-default tip' onclick="borrar(<?php echo $usu->id; ?>,'<?php echo $usu->username; ?>')" title="Eliminar" >
...
<script type="text/javascript">
    ...
    function editar(id) {
        console.log(id);
        $("#agregar").load('<?= $ruta ?>facturador/usuario/form/'+id);
        $('#agregar').modal('show');
    }

    function agregar() {
        $("#agregar").load('<?= $ruta ?>facturador/usuario/form');
        $('#agregar').modal('show');
    }
    ...
    var usuario = {
        ajaxgrupo : function(){
            return  $.ajax({
                url:'<?= base_url()?>facturador/usuario/index'
        ...
        guardar : function () {
            ...
            if ($("#var_usuario_clave").val() == '' && $("#id").val() == '') {
                var growlType = 'warning';
...
<div class="modal fade" id="borrar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <form name="formeliminar" id="formeliminar" method="post" action="<?= $ruta ?>facturador/usuario/eliminar">
